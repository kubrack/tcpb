...
let _log line = fprintf log_ch line
...
let rm_client ex fd = 
  match ex with
  | Unix_error (unix_err, _syscall, _where) -> begin
      let () = eprintf "srv: closing %d: %s\n%!" (int_of_file_descr fd) 
        (error_message unix_err) in
      let () = close fd in
      Lwt.return_none
  end
  | _ -> raise ex
...
let start host port = 
  ...
  let rec cb_accept (fd, addr) = begin
    ...
    let msg =
      ...
      | ADDR_UNIX s -> s
    in
    let () = eprintf "srv: accepted: %s as %d\n%!" msg (int_of_file_descr unix_fd) in
    ...
    let p_read = (fun () -> Lwt_io.read_line in_chan) in
    let rec cb_read line = begin
      let _ = Lwt_io.write_line (Lwt_io.of_fd ~mode:Lwt_io.Output Lwt_unix.stdout) line  (* TODO to log *) in
